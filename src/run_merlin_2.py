...
def train_DNN(train_xy_file_list, valid_xy_file_list, \
    ...
    do_pretraining = hyper_params['pretrain_model']
    ...
    # Json and h5 files to save model
    ...
    # This is just to test the data reader...
    ...
    if cfg.sequential_training == False  and cfg.backend == 'theano':
        dnn = DeepRecurrentNetwork(n_in=n_ins, hidden_layer_size=hidden_layer_size, n_out=n_outs,
    ...
    elif cfg.sequential_training == False and cfg.backend == 'keras':

        dnn = kerasModels(n_in=n_ins,
    ...
    elif cfg.sequential_training == True  and cfg.backend == 'keras':
    ...
    while epoch < training_epochs:
        ...
        while not train_data_reader.is_finish():
            ...
            for index in range(n_train_batches):
                if cfg.backend == 'keras':
                    ...
                    this_train_error = error[0]

                elif cfg.backend == 'theano':
                    ...
                    logger.critical('%s deep learning backend is not supported!' % (cfg.backend))
                    raise
        ...
        while (not valid_data_reader.is_finish()):
            ...
            this_valid_loss = None
            if cfg.backend == 'keras':

                error = dnn.model.test_on_batch(temp_valid_set_x, temp_valid_set_y)
                this_valid_loss = error[0]
...
def dnn_hidden_generation(valid_file_list, nnets_file_name, n_ins, n_outs, out_file_list):
    ...
    for i in range(file_number):
        ...
        predicted_parameter = dnn.generate_top_hidden_layer(test_set_x=test_set_x)
